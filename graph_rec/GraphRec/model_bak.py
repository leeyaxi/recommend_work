...
class GraphRec(nn.Module):
    ...
    def __init__(self, num_users, num_items, num_rate_levels, emb_dim = 64):
        ...
        self.emb_dim = emb_dim
        self.user_emb = nn.Embedding(self.num_users, self.emb_dim, padding_idx = -1)
        self.item_emb = nn.Embedding(self.num_items, self.emb_dim, padding_idx = -1)
        self.rate_emb = nn.Embedding(self.num_rate_levels, self.emb_dim, padding_idx = 0)

        self.user_model = UserModel(self.emb_dim, self.user_emb, self.item_emb, self.rate_emb)

        self.item_model = ItemModel(self.emb_dim, self.user_emb, self.item_emb, self.rate_emb)
    ...
    def get_item_embedding(self, iids, i_user_pad):
        return self.item_model(iids, i_user_pad)

    def item_recall(self, h_i, item_embedding_table):
        bs = h_i.shape[0]
        id_size = item_embedding_table.shape[0] #(item_size, 64)
        h_i = h_i.unsqueeze(1).tile([1, id_size, 1])  #(bs, item_size, 64)
        item_embedding_table = item_embedding_table.unsqueeze(0).tile([bs, 1, 1])
        item_recall_score = self.rate_pred(torch.cat([h_i, item_embedding_table], dim=-1)) #(bs, item_size)
        return item_recall_score.squeeze(2) #(bs, item_size)
